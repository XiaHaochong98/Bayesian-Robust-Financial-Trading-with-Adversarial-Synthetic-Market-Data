...
def main(model, stock):
    ...
    logger = CSVLogger(
        "./", name=f"lightning_logs/downstream/{stock}", version="no_aug"
    )
    ...
    test_prediction_plot(
        f"Real Data Prediction using {model.__class__.__name__} Model",
        logger.log_dir,
        y_test_unscaled,
        test_unified_unscaled,
    )
...
if __name__ == "__main__":
    ...
    for stock in stocks:
        print("Running experiment: ", stock)
        main(model=args["model"], stock=stock)
