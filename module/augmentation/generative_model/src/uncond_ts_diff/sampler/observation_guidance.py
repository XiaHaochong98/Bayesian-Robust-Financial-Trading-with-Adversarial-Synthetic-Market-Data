...
PREDICTION_INPUT_NAMES_NO_DYNAMIC = [
    "past_target",
    "past_observed_values",
    "feat_static_cat",
    "feat_static_real",
...
class Guidance(torch.nn.Module):
    ...
    def forward(
        ...
        past_feat_dynamic_real: torch.Tensor = None,
        future_feat_dynamic_real: torch.Tensor = None,
    ):
        device = next(self.model.parameters()).device
        # print("Past Dynamic Real: ", past_feat_dynamic_real.shape)
        # print("Future Dynamic Real: ", future_feat_dynamic_real.shape)
        future_target = torch.zeros(
            past_target.shape[0], self.prediction_length, device=device
        )
        data = dict(
            feat_static_cat=(
                feat_static_cat.to(device)
                if feat_static_cat is not None
                else None
            ),
            feat_static_real=(
                feat_static_real.to(device)
                if feat_static_real is not None
                else None
            ),
            past_time_feat=(
                past_time_feat.to(device)
            ...
            past_observed_values=(
                past_observed_values.to(device)
                if past_observed_values is not None
                else None
            ),
            future_time_feat=(
                future_time_feat.to(device)
                if future_time_feat is not None
                else None
            ),
            past_feat_dynamic_real=(
                past_feat_dynamic_real.to(device)
                if past_feat_dynamic_real is not None
                else None
            ),
            future_feat_dynamic_real=(
                future_feat_dynamic_real.to(device)
    ...
    def get_predictor(self, input_transform, batch_size=40, device=None):
        if self.num_feat_dynamic_real > 0:
            ...
            input_names = PREDICTION_INPUT_NAMES_NO_DYNAMIC
