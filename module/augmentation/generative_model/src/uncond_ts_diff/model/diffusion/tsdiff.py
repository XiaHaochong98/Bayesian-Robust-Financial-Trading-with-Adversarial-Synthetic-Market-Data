...
class TSDiff(TSDiffBase):
    def __init__(
        ...
        print(
            "Number of Features in Backbone",
    ...
    def _extract_features(self, data):
        # print("Data Prior", data["past_target"].shape)
        ...
        if data["feat_static_cat"] is not None:
            # print("feat_static_cat", data["feat_static_cat"].shape)
        ...
        if data["past_time_feat"] is not None:
            # print("past_time_feat", data["past_time_feat"].shape)
            time_features.append(
                data["past_time_feat"][:, -self.context_length :]
            )
        if data["future_time_feat"] is not None:
            # print("future_time_feat", data["future_time_feat"].shape)
            time_features.append(data["future_time_feat"])
        features.append(torch.cat(time_features, dim=1))
        # print("Shape of features", features[0].shape, features[1].shape)

        dynamic_time_features = []
        if data.get("past_feat_dynamic_real", None) is not None:
            # print(
            #     "past_feat_dynamic_real", data["past_feat_dynamic_real"].shape
            # )
            dynamic_time_features.append(
                data["past_feat_dynamic_real"][:, -self.context_length :]
            )
        if data.get("future_feat_dynamic_real", None) is not None:
            # print(
            #     "future_feat_dynamic_real",
            #     data["future_feat_dynamic_real"].shape,
            # )
            dynamic_time_features.append(data["future_feat_dynamic_real"])
        if len(dynamic_time_features) > 0:
            # print(data)
            # print("Adding dynamic time features", len(dynamic_time_features))
            features.append(torch.cat(dynamic_time_features, dim=1))
        # print(
        #     "Shape of features",
        #     features[0].shape,
        #     features[1].shape,
        ...
        if self.use_lags:
            # print("Lagg Seq", self.lags_seq)
            # print("Scaled Prior", scaled_prior.shape)
            # print("Scaled Context", scaled_context.shape)
            # print("Scaled Future", scaled_future.shape)
            # print(
            #     "Cat",
            #     (torch.cat([scaled_context, scaled_future], dim=1)).shape,
            # )
