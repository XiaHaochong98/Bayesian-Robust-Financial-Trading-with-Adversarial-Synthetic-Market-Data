...
def rolling_window_normalization(train_data, test_data, window_size):
    # TODO: check if this cause data leakage
    # Assuming train_data and test_data are numpy arrays of shape (no, seq_len, dim)
    no_train, seq_len_train, dim = train_data.shape
    no_test, seq_len_test, _ = test_data.shape

    # Normalized data initialization
    normalized_train_data = np.zeros_like(train_data)
    normalized_test_data = np.zeros_like(test_data)

    # Apply rolling window normalization
    for start in range(0, seq_len_train - window_size):
        end = start + window_size
        scaler = StandardScaler()

        # Reshape for fitting the scaler
        scaler.fit(train_data[:, start:end, :].reshape(-1, dim))

        # Transform both train and test data in this window
        normalized_train_data[:, start:end, :] = scaler.transform(train_data[:, start:end, :].reshape(-1, dim)).reshape(
            -1, window_size, dim)
        if start < seq_len_test - window_size:  # ensure the window is within test data's range
            normalized_test_data[:, start:end, :] = scaler.transform(
                test_data[:, start:end, :].reshape(-1, dim)).reshape(-1, window_size, dim)
...
def data_sample_worker(data, seq_len, features, history_length, full_data, tic, dynamics, args):
    ...
    # split the all the variables into train and test with configs.train_rate
